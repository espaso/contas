var RadialBar = function(ctx, config) {
	...
	this.backLineFill = config.backLineFill || '#fff';
	this.bgFill = config.bgFill || 'transparent';

	this.infoStyle = config.infoStyle || '20px';
	this.infoColor = config.infoColor || '#909090';
...
RadialBar.prototype.update = function() {
	this.ctx.save();
		...
		this.ctx.rotate(this.radians(270));
		...
		this.ctx.beginPath();
		this.ctx.arc(0, 0, this.radius, this.radians(this.degProgress), this.radians(0), true);
		this.ctx.stroke();
