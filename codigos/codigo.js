...
function Remuneracao(holerite) {
	var noturno = ((holerite/180)*150)*0.3;
	var henot = ((((holerite/180)*8)*21)*(60/52.5))*0.2;
	var dsr = (noturno+henot)/5.5;
	var bruto = holerite+noturno+henot+dsr;
	var adia = 0;//holerite*0.4;
	var inss = (1621*0.075)+((2902.84-1621)*0.09)+((4354.27-2902.85)*0.12)+((bruto-4354.28)*0.14);
	var irrf = ((bruto*0.275)-908.73);
	var desconto = adia+inss+irrf;
	var liquido = (bruto)-(desconto);
	//alert("Adicional = "+noturno+" - HENot = "+henot+" - DSR = "+dsr+" - Sal Bruto = "+bruto+" - INSS = "+inss+" - IRRF =  "+irrf+" - Desconto = "+desconto+" - Sal Liquido = "+liquido);
	return liquido;
}
...
if (window.AndroidBridge) {
	// Chama o método Java diretamente e armazena o retorno na variável texto
	let correnteBruto = window.AndroidBridge.lerArquivoContas("corrente.js");
	let mensalBruto = window.AndroidBridge.lerArquivoContas("mensal.js");
	let passadoBruto = window.AndroidBridge.lerArquivoContas("passado.js");

    try {
        // 2. Remove o "var pegaCorrente =" e possíveis espaços ou quebras de linha
        let correnteLimpo = correnteBruto.replace(/^(var\s+pegaCorrente\s*=\s*)/, '');
        let mensalLimpo = mensalBruto.replace(/^(var\s+pegaMensal\s*=\s*)/, '');
        let passadoLimpo = passadoBruto.replace(/^(var\s+pegaPassado\s*=\s*)/, '');
          
        // 3. Remove o ponto e vírgula do final, se houver
        correnteLimpo = correnteLimpo.trim().replace(/;$/, '');
		mensalLimpo = mensalLimpo.trim().replace(/;$/, '');
		passadoLimpo = passadoLimpo.trim().replace(/;$/, '');

        // 4. Agora sim, faz o parse do JSON purificado
        if (correnteLimpo.startsWith("'") && correnteLimpo.endsWith("'")) {
            correnteLimpo = correnteLimpo.slice(1, -1);
        }
		
		if (mensalLimpo.startsWith("'") && mensalLimpo.endsWith("'")) {
            mensalLimpo = mensalLimpo.slice(1, -1);
        }
		
		if (passadoLimpo.startsWith("'") && passadoLimpo.endsWith("'")) {
            passadoLimpo = passadoLimpo.slice(1, -1);
        }

        // 4. PRIMEIRO PARSE: Transforma a string limpa no texto do JSON real
        // 5. SEGUNDO PARSE: Transforma o texto do JSON no array de objetos utilizável
		var TabCorrente = JSON.parse(correnteLimpo);
        if (typeof TabCorrente === 'string') {
            TabCorrente = JSON.parse(TabCorrente);
        }
		
		var TabMensal = JSON.parse(mensalLimpo);
		if (typeof TabMensal === 'string') {
            TabMensal = JSON.parse(TabMensal);
        }
		
		var TabPassado = JSON.parse(passadoLimpo);
		if (typeof TabPassado === 'string') {
            TabPassado = JSON.parse(TabPassado);
        }
    } catch (erro) {
        alert("Erro ao processar ou limpar o arquivo:", erro);
    }
}
*/